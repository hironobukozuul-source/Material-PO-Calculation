...
def load_sg_cap_mappings(uploaded_file):
    try:
        ...
        for sheet in xl.sheet_names:
            if sheet.upper() == 'SG-CAP':
                target_sheet = sheet
                break
        if not target_sheet:
            return {}
        df_cap = pd.read_excel(uploaded_file, sheet_name=target_sheet, header=None)
        mapping = {}
        for r in range(len(df_cap)):
            if len(df_cap.columns) > 6:
                sku_val = df_cap.iloc[r, 3]
                weight_val = df_cap.iloc[r, 6]
...
def process_tasks(df_raw):
    ...
    for i in range(3, len(df_raw)):
        if DATE_COL >= len(df_raw.columns): continue
        ...
        for line, cols in line_config.items():
            if cols['ton'] >= len(df_raw.columns): continue
    ...
    if available_weeks:
        ...
        if st.button("🚀 レポート生成開始"):
            with st.spinner('データを最適化処理中...'):
                ...
                # --- HIGHLY OPTIMIZED RECALCULATION ENGINE ---
                ...
                clean_tasks = df_tasks[~df_tasks['is_maint']].copy()
                week_end_dt = start_dt_excel + datetime.timedelta(hours=TOTAL_HOURS)
                clean_tasks = clean_tasks[(clean_tasks['Finish'] >= start_dt_excel) & (clean_tasks['Start'] <= week_end_dt)]
                ...
                for i, h_dt in enumerate(hour_list):
                    ...
                    c1 = ws_vol.cell(1, 3 + i, header_str)
                    c1.font = Font(bold=True); c1.alignment = Alignment(text_rotation=90, horizontal='center')
                ...
                for r_idx, (line, product) in enumerate(unique_items):
                    ...
                    sku_clean = str(product).strip()
                    filling_grams = sg_cap_map.get(sku_clean, None)
                    matched_tasks = clean_tasks[(clean_tasks['Line'] == line) & (clean_tasks['Product'] == product)]
                    ...
                    for c_idx, h_dt in enumerate(hour_list):
                        ...
                        for _, t in matched_tasks.iterrows():
                            overlap_start = max(t['Start'], h_dt)
                            overlap_end = min(t['Finish'], h_end)
                            overlap_sec = (overlap_end - overlap_start).total_seconds()
                            if overlap_sec > 0:
                                total_sec = (t['Finish'] - t['Start']).total_seconds()
                                if total_sec > 0:
                                    ton_sum += t['Ton'] * (overlap_sec / total_sec)
                        
                        calc_pcs = 0.0
                        if ton_sum > 0:
                            cell_v = ws_vol.cell(row_num, 3 + c_idx, round(ton_sum, 2))
                            cell_v.fill = PatternFill(start_color="D9EAD3", end_color="D9EAD3", fill_type="solid")
                            
                            if filling_grams:
                                calc_pcs = (ton_sum * 1000000) / filling_grams
                                cell_p = ws_pcs.cell(row_num, 3 + c_idx, int(round(calc_pcs)))
                            else:
                                cell_p = ws_pcs.cell(row_num, 3 + c_idx, "Missing Spec")
                            cell_p.fill = PatternFill(start_color="C9DAF8", end_color="C9DAF8", fill_type="solid")
                        
                        row_hourly_pcs.append(calc_pcs)
